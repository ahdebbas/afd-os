...
// Recent WHOOP workouts (kJ -> kcal), newest first. Needs read:workout scope.
export async function fetchWorkouts(accessToken, days = 7) {
  const start = new Date(Date.now() - days * 24 * 3600 * 1000).toISOString()
  const r = await fetch(`${WHOOP_API}/v2/activity/workout?start=${encodeURIComponent(start)}&limit=15`, {
    headers: { Authorization: `Bearer ${accessToken}` },
  })
  if (!r.ok) throw new Error(`WHOOP workouts ${r.status}`)
  const data = await r.json()
  return (data.records || [])
    .map(w => ({
      id: w.id,
      ...
      start: w.start,
      durationMin: w.start && w.end ? Math.round((new Date(w.end) - new Date(w.start)) / 60000) : null,
