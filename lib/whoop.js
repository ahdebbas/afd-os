...
export async function intradayComparison(uid) {
  ...
  const { data, error } = await admin()
    .from('whoop_samples')
    .select('captured_at,kcal')
    .eq('user_id', uid)
    .gte('captured_at', since)
    .order('captured_at', { ascending: true })
  if (error || !data?.length) return { yesterday: null, weeklyAvg: null, days: 0, hour: new Date().getUTCHours() }
  ...
  return { yesterday, weeklyAvg, days: vals.length, hour }
}
